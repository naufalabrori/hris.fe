...
import { useListPayroll } from '@/hooks/Services/Finance/Payroll/useGetPayrolls';
...
function useTableData<TData>(
  ...
  const { data: queryData, error, isLoading } = useListPayroll(params);
...
export function PayrollDataTable({ employeeId }: { employeeId?: string }) {
  const [totalData, setTotalData] = useState(0);
  const [limit, setLimit] = useState<string>('5');
  const [sorting, setSorting] = useState<SortingState>([{ id: 'createdDate', desc: true }]);
  const [filterBy, setFilterBy] = useState<string>('employeeName');
  ...
  const { data, total, loading, error } = useTableData<Payroll>(
    sorting,
    pagination,
    {
      key: filterBy,
  ...
  const columns: ColumnDef<any, Payroll>[] = PayrollColumns({
    currentPage,
  ...
  const dataColumnFiltering = [{ key: 'employeeName', value: 'Employee Name' }];
