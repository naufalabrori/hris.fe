...
export function MenuDataTable() {
  ...
  const [filterBy, setFilterBy] = useState<string>('menuName');
  ...
  const { data, total, loading, error } = useTableData<Menu>(sorting, pagination, {
    key: filterBy,
    value: debounceFilter,
  });

  const columns: ColumnDef<any, Menu>[] = MenuColumns({
    currentPage,
  ...
  const dataColumnFiltering = [{ key: 'menuName', value: 'Menu Name' }];
