...
function useTableData<TData>(
  ...
  const params: any = {
    ...pagination,
    sortBy: sortField?.id || 'resource',
    isDesc: sortField?.desc || false,
    [filter.key]: filter.value || '',
  };

  const { data: queryData, error, isLoading } = useListPermission(params);
...
export function PermissionDataTable() {
  ...
  const { data, total, loading, error } = useTableData<Permission>(sorting, pagination, {
    key: filterBy,
    value: debounceFilter,
  });

  const columns: ColumnDef<any, Permission>[] = PermissionColumns({
    currentPage,
