...
function useTableData<TData>(
  ...
  const { data: queryData, error, isLoading } = useListEmployee(params);
...
export function EmployeeUserDataTable({ dataMaster }: { dataMaster: any }) {
  const [totalData, setTotalData] = useState(0);
  const [limit, setLimit] = useState<string>('5');
  const [sorting, setSorting] = useState<SortingState>([{ id: 'firstName', desc: true }]);
  ...
  const { data, total, loading, error } = useTableData<Employee>(sorting, pagination, {
    key: filterBy,
  ...
  const columns: ColumnDef<any, Employee>[] = EmployeeColumns({
