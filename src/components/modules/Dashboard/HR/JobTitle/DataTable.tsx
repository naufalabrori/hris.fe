...
import { JobTitleColumns } from './Columns';
...
function useTableData<TData>(
  ...
  const { data: queryData, error, isLoading } = useListJobTitle(params);
...
export function JobTitleDataTable() {
  ...
  const { data, total, loading, error } = useTableData<JobTitle>(sorting, pagination, {
    key: filterBy,
    value: debounceFilter,
  });

  const columns: ColumnDef<any, JobTitle>[] = JobTitleColumns({
    currentPage,
