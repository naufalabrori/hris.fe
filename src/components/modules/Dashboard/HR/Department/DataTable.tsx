...
function useTableData<TData>(
  ...
  const { data: queryData, error, isLoading } = useListDepartment(params);
...
export function DepartmentDataTable() {
  ...
  const [filterBy, setFilterBy] = useState<string>('departmentName');
  ...
  const { data, total, loading, error } = useTableData<Department>(sorting, pagination, {
    key: filterBy,
    value: debounceFilter,
  });

  const columns: ColumnDef<any, Department>[] = DepartmentColumns({
    currentPage,
