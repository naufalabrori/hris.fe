...
import { useListTraining } from '@/hooks/Services/HumanResource/Training/useGetTrainings';
...
function useTableData<TData>(
  ...
  const { data: queryData, error, isLoading } = useListTraining(params);
...
export function TrainingDataTable() {
  ...
  const [filterBy, setFilterBy] = useState<string>('trainingName');
  ...
  const { data, total, loading, error } = useTableData<Training>(sorting, pagination, {
    key: filterBy,
    value: debounceFilter,
  });

  const columns: ColumnDef<any, Training>[] = TrainingColumns({
    currentPage,
  ...
  const dataColumnFiltering = [
    { key: 'trainingName', value: 'Training Name' },
