...
function useTableData<TData>(
  ...
  const { data: queryData, error, isLoading } = useListBenefit(params);
...
export function BenefitDataTable({ employeeId }: { employeeId: string }) {
  const [totalData, setTotalData] = useState(0);
  const [limit, setLimit] = useState<string>('5');
  const [sorting, setSorting] = useState<SortingState>([{ id: 'createdDate', desc: true }]);
  const [filterBy, setFilterBy] = useState<string>('benefitType');
  ...
  const { data, total, loading, error } = useTableData<Benefit>(
    sorting,
    pagination,
    {
      key: filterBy,
  ...
  const columns: ColumnDef<any, Benefit>[] = BenefitColumns({
    currentPage,
  ...
  const dataColumnFiltering = [
    { key: 'benefitType', value: 'Benefit Type' },
