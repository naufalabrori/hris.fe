...
import { useListLeave } from '@/hooks/Services/Activity/Leave/useGetLeaves';
...
function useTableData<TData>(
  ...
  const { data: queryData, error, isLoading } = useListLeave(params);
...
export function LeaveDataTable({ employeeId }: { employeeId?: string }) {
  const [totalData, setTotalData] = useState(0);
  const [limit, setLimit] = useState<string>('5');
  const [sorting, setSorting] = useState<SortingState>([{ id: 'createdDate', desc: true }]);
  const [filterBy, setFilterBy] = useState<string>('leaveType');
  ...
  const { data, total, loading, error } = useTableData<Leave>(
    sorting,
    pagination,
    {
      key: filterBy,
  ...
  const columns: ColumnDef<any, Leave>[] = LeaveColumns({
    currentPage,
  ...
  const dataColumnFiltering = [{ key: 'leaveType', value: 'Leave Type' }];
